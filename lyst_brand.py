...
def get_prev_brands():
	results = set()
	with open('files/lyst_brands.csv', 'r', encoding='utf-8') as fp:
...
def start_parse():
	...
	with open('files/lyst_brands_p2.csv', 'a', encoding='utf-8', newline='\n') as fa:
		writer_a = csv.DictWriter(fa, fieldnames=['brand', 'retailer', 'retailer_slug', 'retailer_brand_href'])
		writer_a.writeheader()

		# with open('files/lyst_brands_failed_p2.csv', 'a', encoding='utf-8', newline='\n') as fe:
		# 	writer_e = csv.DictWriter(fe, fieldnames=['brand', 'url'])
		# 	writer_e.writeheader()
		#
		# 	with open('files/lyst_brands_empty.csv', 'a', encoding='utf-8', newline='\n') as fem:
		...
		for brand_slug, url in urls.items():
			...
			try:
				...
				if not d['data']['options']:
					empties.append((brand, f'https://www.lyst.com/designer/{brand_slug}/'))
